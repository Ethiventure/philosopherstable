...
- **OpenRouter**: your key. *Free cycle* hops across free models each turn so limits stretch; *Paid model* pins one ID (default `z-ai/glm-5.3-flash`, ~$0.075/$0.25 per 1M — about a cent a session). Test-key button included; keys live in your browser only, never touch our servers (OpenRouter's free models may log prompts for training — the panel says so).
