...
- **Groq / DeepInfra / Together / Alibaba**: your key, picked or typed models (`qwen3.8-27b`, `Qwen3-30B-A3B`, Model Studio codes). Groq's free tier needs no card — but speaks Low only (its ~7k-token wall can't fit bigger prompts; Medium/High are blocked outright). DeepInfra and Together want a card on file.
- What we'd pick (Sep 26 verdicts, prices are estimates from average tokens per sitting at Sep 26 API rates): quality `qwen3.8-max` via Alibaba (~$0.40) · value `qwen3.7-plus` via Alibaba (~$0.07) · free Groq Low or the OpenRouter free cycle ($0, when unthrottled).
...
Python side (eval tooling only — the app itself needs just Node): Python 3.12 via `uv python install 3.12`, envs live in `.venv312/` (current) and `.venv/` (old ingest env, kept until the new one proves itself). Neither is committed — recreate with `uv venv --python 3.12 .venv312` plus the package set. Phase 7b metric installs (torch et al, CPU-only, ~2GB downloads) target `.venv312`; any Apple-Silicon Mac runs them, slowly — no GPU, no server, no paid anything.
...
AGPL-3.0-only (see `LICENSE`): run it, study it, share it — any public use must share its code too. The corpus texts keep their own licences (see the manifest).
