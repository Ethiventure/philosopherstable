...
| Sep 2026 | OpenRouter free cycle | Qwen/DeepSeek family-first: `qwen/qwen3-next-80b-a3b-instruct:free`, `qwen/qwen3-coder:free`, `deepseek/deepseek-v4-flash:free` (404 since Jun 2026, kept first-try), then `openrouter/free` router mid-list, then Gemma/Nemotron/Nex/Laguna/Ling/North/Liquid bench in adherence order | Works; DeepSeek :free currently 404s and skips fast. IDs rot in days — recheck via `/models` API. Router can return OpenAI `gpt-oss` (standing rule bans OpenAI) — pins exist partly to avoid reaching it. |
