...
| Sep 19 2026 | DeepInfra | `Qwen/Qwen3-30B-A3B` as primary (re-pin, no live grade yet) | Replaces the burned 3.6-35B on the Qwen radio per owner order. Why this ID: live on DeepInfra ($0.12/$0.50, Flex $0.096/$0.40), JSON + function calling, Apache 2.0, hybrid thinker with a documented `enable_thinking:false` switch — and the pipe already sends that pass-through plus `reasoning_effort:low`. Same switch failed to save the 35B, so this gets exactly one live session before trust; burn watch stands. Settings now shows model + one-line why per option (transparency rule, AGENTS.md). |
