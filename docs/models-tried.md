...
| Sep 2026 | DeepInfra | `Qwen/Qwen3.6-35B-A3B` ($0.10/$0.95, MoE 3B-active, JSON+function) added as choosable primary vs DeepSeek V4 Flash 0731; Llama backup rescues either | Untested live — thinking-burn watch item (thinks by default, no dampening param sent). Owner's cheap-vs-cheap A/B vehicle. |
| Sep 2026 | Groq | `qwen3.8-27b` flagged slow in UI (14min + 3 resumes/session observed); `qwen3.6-27b` positioned as fast alternative | Owner testing 3.6 next. No cheap 3.8 exists (3.8-27B ~$0.425/$2.55, Max $1.65/$4.99) — 3.8 is free-slow or pricey, no middle. |
