...
| Sep 2026 | OpenRouter free cycle | Router-first + 13-model named bench (all verified live via `/models` API Sep 15 2026, 20 free total). Removed: `qwen/qwen3-coder:free` (coder-tuned, wrong shape), `deepseek/deepseek-v4-flash:free` (404 since Jun 2026), `qwen/qwen3-next-80b-a3b-instruct:free` (also gone). Zero Qwen/DeepSeek `:free` on the API today — no free DeepSeek alternative exists; family A/B must run on paid/cheap pins. | Router leads so relists auto-rejoin; last-good holds one voice after. Untested new arrivals NOT in cycle: `dots-studio/dots-3-note-preview:free`, `inclusionai/ling-3.0-flash-vl:free`, `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free`, `thinkingmachines/inkling:free` + `:small`, `z-ai/glm-5.2:free` (relisted after 404 — needs re-verify, do not trust yet). Excluded: `nvidia/nemotron-3.5-content-safety:free` (filter model). Paid pin `deepseek/deepseek-v4.1-flash` verified LIVE same day. |
...
| Sep 2026 | DeepInfra | `deepseek-ai/DeepSeek-V4-Flash-0731` ($0.06/$0.18) primary, `meta-llama/Llama-3.3-70B-Instruct-Turbo` automatic backup | Fixed order, no setting: V4 Flash speaks first; Llama rescues the turn on any non-auth/quota failure. Export provenance records who spoke, so the adherence A/B accumulates organically. |
