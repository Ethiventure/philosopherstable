          ...
          <div className="space-y-3 border-b border-[#4a392d]/15 pb-6 mb-6">
            ...
            {settings.provider === 'shared' ? (
              ...
              <>
                ...
                <p className="text-xs text-[#465f75]/70">Groq retires IDs without notice — type a current one (see <a className="underline" href="https://console.groq.com/docs/models" target="_blank" rel="noreferrer">console.groq.com/docs/models</a>) and press Test key. Free tier starts fast but walls at ~7K input tokens per request, so sessions stutter (Sep 2026). Past picks: <span className="font-heading">qwen/qwen3.8-27b</span> (best voice, slow on free tier), <span className="font-heading">qwen/qwen3.6-27b</span> (dead for visitor keys since Sep 2026).</p>
              </>
            ) : usingOpenRouter ? (
              <>
                ...
                {settings.openRouterMode === 'free' ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-[#465f75]/70">Higher quality with much bigger limits than free — needs credits on your key. Default <span className="font-heading">deepseek/deepseek-v4.1-flash</span> ($0.15/$0.60 per 1M, 1M context): fastest reliable engine measured here (~6 min sessions, ~$0.01 each). For voice over pace, type <span className="font-heading">qwen/qwen3.8-27b</span> (~$0.20+/session). Browse non-OpenAI IDs at <a className="underline" href="https://openrouter.ai/models" target="_blank" rel="noreferrer">openrouter.ai/models</a>. If a paid model reports a daily limit, check the key's own cap at <a className="underline" href="https://openrouter.ai/keys" target="_blank" rel="noreferrer">openrouter.ai/keys</a> — new credit can take minutes to apply.</p>
                  </>
                )}
              </>
            ) : usingDeepInfra ? (
              ...
              <>
                ...
                <p className="text-xs text-[#465f75]/70">Model Studio codes vary by region — type the exact code and press Test key. Free quota pools in Singapore; enable Stop-on-Exhaust so overruns stop instead of billing. Trial quota ends Dec 16 2026 — check remaining quota in Model Studio before a big sitting. Default <span className="font-heading">qwen3.8-27b</span> runs unthinking: best voice measured here, seconds per turn (Sep 2026).</p>
              </>
            ) : usingZai ? (
              <>
                ...
                <p className="text-xs text-[#465f75]/70">Free tier, no card — phone verification only. Default <span className="font-heading">glm-4.7-flash</span> ($0 in/out) is free but allows 1 request at a time, so sittings run slow. Reasoning stays on (cannot be disabled) — the output-token line is the judge on burn. Get a key at <a className="underline" href="https://z.ai/" target="_blank" rel="noreferrer">z.ai</a>.</p>
              </>
            ) : (
              <>
