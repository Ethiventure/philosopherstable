...
export async function searchThinkerPassages(
  ...
  const header = intensity === 'low'
    ...
    : intensity === 'high'
      ? `INDEXED PASSAGES from '${work.title}' [${number}] — searched from this thinker's own indexed works for this question. Quote generously: weave at least four distinctive single words or short phrases (no more than six words each, in single quotes — bare double quotes corrupt your reply) from these passages into your own sentences, and cite each use [${number}]. Echo their filler words, diction tics, and rhythms. Reaching for their less famous vocabulary beats restating their greatest hits: prefer the passage's own terms over your stock summary of this thinker. What you don't quote, closely paraphrase, always citing [${number}]:`
