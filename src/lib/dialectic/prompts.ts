...
export function buildTurnInstruction({ kind, prevName, isFinalSeat, longForm, reversed = false, marginsNote = false, marginsEarly = false, marginsFirst = false, lowRegister = false, intensity, heat, threadCity = null, pass }: TurnInstructionArgs): string {
  ...
  return [
    ...
    ...(low
      ...
      : (!reversed
        ? [`CUT IN, don't hand over: seize the weakest point in ${prev}'s closing lines. Open with ${prev} by name in the YOU–I shape (see NEGATION below) — no other preamble, no greeting. Never open with a summarising You-verb (think, focus, see, suggest, argue, claim, believe); You-verbs are welcome when they are your toolkit verbs (you distinguish, expose, trace, rescue, sublate).`]
        : [])),
    `1. DETERMINATE NEGATION (roughly ${b.negation} words, ONE paragraph, 2–4 sentences): open with PREV by name in this exact shape — ‘${prev}, you [said-synonym: showed, saw, caught, granted] X and Y, I [reject-synonym] Y and [inject-synonym] Z.’ Then steelman ${prev}'s claim at its strongest — then expose its internal contradiction: where ${prev}'s own claims, premises, or tensions collide with each other — but as TRANSLATION: restate it wholly in your framework's own vocabulary, no clause over five words matching ${prev} verbatim.${low ? ' At Low there are no shared specialist terms: restate everything, including school-terms, in plain everyday words.' : ' Single shared terms (class struggle, decreation) may repeat; multi-word clauses may not.'}${level === 'medium' ? ' DICTION IN THIS SECTION: every school-term kept here carries its plain meaning inside its sentence plus one short concrete sentence showing what it does — term, gloss, elaboration, no bare terms. Append a fifth JSON key "glossary": each hard term you used, one line each as term — plain meaning.' : ''} The keep and the break live in that opening YOU–I movement — YOU is always PREV, I is always you the speaker; any other assignment fails the turn. Everywhere else address them as YOU, a live opponent, not a specimen. If you agree with them you have misread them.`,
    reformulationLine,
    'QUESTION RULE: paraphrase the question through your framework — never repeat any multi-word clause of it verbatim.',
    'ECHO RULE: answer PREV — never restate PREV, yourself, or the question. No sentence may reword an earlier sentence of theirs or yours; avoid even repeating ideas — each sentence must push the debate in a new direction. Shared paragraphs fail outright: never reuse PREV’s example, image, slogan, or scene — bring your own concrete object. A turn that circles has failed, even if every word differs.',
